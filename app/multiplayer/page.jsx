...
export default function CodingChallengePage({ mode, difficulty, player1, player2 }) {
    const [code, setCode] = useState("// Write your code here"); // Default code
    const [output, setOutput] = useState(""); // Output from backend
    const [language, setLanguage] = useState("javascript"); // Default language
    const [loading, setLoading] = useState(false); // Loading state
    const [timeLeft, setTimeLeft] = useState(300); // Timer (5 minutes)
    const [playerProgress, setPlayerProgress] = useState({ player1: 0, player2: 0 }); // Player progress
    const [question, setQuestion] = useState(null); // Question state
    const editorRef = useRef(null); // Reference to Monaco Editor instance

    // Fetch question when difficulty changes
    useEffect(() => {
        ...
        fetchQuestion();
    }, [difficulty]); // Trigger when difficulty changes
    ...
    useEffect(() => {
        if (timeLeft > 0) {
            const timer = setInterval(() => {
                ...
                updatePlayerProgress(); // Update progress for both players
            }, 1000);
            return () => clearInterval(timer);
        }
    }, [timeLeft]);

    // Function to update progress for both players (mocked here)
    ...
    // Run code by calling FastAPI backend
    const runCode = async () => {
        setLoading(true);
    ...
    return (
        <div className="flex flex-col h-screen bg-gray-900 text-white" style={{ height: "100vh" }}>
            ...
            <div className="p-2 bg-gray-800">
                {[player1, player2].map((playerName, index) => (
